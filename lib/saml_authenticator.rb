class SamlAuthenticator < ::Auth::OAuth2Authenticator
  ...
  def register_middleware(omniauth)
    omniauth.provider :saml,
                      name: name,
                      issuer: GlobalSetting.try(:saml_base_url) || Discourse.base_url,
                      idp_sso_target_url: setting(:target_url),
                      idp_slo_target_url: setting(:slo_target_url),
                      slo_default_relay_state: GlobalSetting.try(:saml_base_url) || Discourse.base_url,
                      ...
                      assertion_consumer_service_url: (GlobalSetting.try(:saml_base_url) || Discourse.base_url) + "/auth/#{name}/callback",
                      single_logout_service_url: (GlobalSetting.try(:saml_base_url) || Discourse.base_url) + "/auth/#{name}/slo",
